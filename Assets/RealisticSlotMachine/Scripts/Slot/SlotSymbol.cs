...
namespace Mkey
{
    public class SlotSymbol : MonoBehaviour
    {
        ...
        public SlotGroupBehavior slotGroup;

        public bool isInWindow = false;
        public double currAngle = 0;
        public double tape;
        public SlotSymbol Next { get; set; }
        public SlotSymbol Prev { get; set; }
        ...
        private SlotGroupBehavior Reel { get; set; }
        private WinSymbolBehavior wsb;
        bool blurredState = false;
        #endregion temp vars
        ...
        internal void InitUpdate(bool blur) // update from slotgroup behavior
        {
            blurredState = blur;
            SetIcon(blur);
            UpdateAction?.Invoke();
        ...
        internal void SetIcon(SlotIcon icon, int iconID)
        {
            this.IconID = iconID;
            this.Icon = icon;
            SetIcon(blurredState);
            if (debug) name = tape + " : " + icon.iconSprite.name;
        }
